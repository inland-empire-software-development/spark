...
export class NoVersion1589829911139 implements MigrationInterface {
    name = 'NoVersion1589829911139'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `course_demo` DROP COLUMN `version`", undefined);
        await queryRunner.query("ALTER TABLE `user_meta_demo` DROP COLUMN `version`", undefined);
        await queryRunner.query("ALTER TABLE `user_demo` DROP COLUMN `version`", undefined);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `user_demo` ADD CONSTRAINT `FK_0768215971c9d6f9b57db33c693` FOREIGN KEY (`userMetaId`) REFERENCES `user_meta_demo`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION", undefined);
        await queryRunner.query("ALTER TABLE `user_demo` ADD `version` int NOT NULL", undefined);
        await queryRunner.query("ALTER TABLE `user_meta_demo` ADD `version` int NOT NULL", undefined);
