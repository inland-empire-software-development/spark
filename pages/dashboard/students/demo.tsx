...
const mapDataToCourseProps = (data: any): ManageStudentsCourse[] => {
  return data.map((course: any) => ({
    ...
    code: course.code,
    name: course.name,
    students: course.users.map((student: any) => ({
      id: student.id,
      email: student.email,
      avatarUrl: student.userMeta.avatarUrl,
      firstName: student.userMeta.firstName,
      lastName: student.userMeta.lastName,
      status: student.userMeta.status,
    })),
  }));
};
...
const Students = () => {
  ...
  useEffect(() => {
    fetch(process.env.HOST + "api/course-students/demo", {
      method: 'GET',
      headers: {
